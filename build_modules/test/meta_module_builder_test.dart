...
void main() {
  ...
  test('can serialize meta modules', () async {
    ...
    await testBuilder(MetaModuleBuilder(testPlatform), {
      'a|lib/a.module.library': ModuleLibrary.fromSource(assetA, '').serialize(),
    }, outputs: {
      'a|lib/${metaModuleExtension(testPlatform)}': encodedMatchesMetaModule(metaA),
    });
  });
  ...
  group('isSupported', () {
    test('is false for libraries that import dart:io on web', () async {
      ...
      await testBuilder(MetaModuleBuilder(testPlatform), {
        'a|lib/a.module.library': ModuleLibrary.fromSource(assetA, '').serialize(),
        'a|lib/b.module.library': ModuleLibrary.fromSource(assetB, "import 'dart:io';").serialize(),
      }, outputs: {
        'a|lib/${metaModuleExtension(testPlatform)}': encodedMatchesMetaModule(metaA),
      });
    });

    test('is false for connected components that import dart:io on web', () async {
      var assetA = AssetId('a', 'lib/a.dart');
      var assetB = AssetId('a', 'lib/b.dart');
      var moduleA = Module(assetA, [assetA, assetB], <AssetId>[], testPlatform, false);
      var metaA = MetaModule([moduleA]);
      await testBuilder(MetaModuleBuilder(testPlatform), {
        'a|lib/a.module.library': ModuleLibrary.fromSource(assetA, "import 'b.dart';").serialize(),
        'a|lib/b.module.library':
            ModuleLibrary.fromSource(assetB, "import 'dart:io';import 'a.dart';").serialize(),
      }, outputs: {
        'a|lib/${metaModuleExtension(testPlatform)}': encodedMatchesMetaModule(metaA),
      });
    });
    ...
    test('is false for coarse modules that import dart:io on web', () async {
      ...
      await testBuilder(MetaModuleBuilder(testPlatform), {
        'a|lib/a.module.library':
            ModuleLibrary.fromSource(assetA, "import 'src/b.dart';").serialize(),
    ...
    test('does not look at dependencies', () async {
      var assetA = AssetId('a', 'lib/a.dart');
      var assetB = AssetId('a', 'lib/b.dart');
      var moduleA = Module(assetA, [assetA], <AssetId>[assetB], testPlatform, true);
      var moduleB = Module(assetB, [assetB], <AssetId>[], testPlatform, false);
      var metaA = MetaModule([moduleA, moduleB]);
      await testBuilder(MetaModuleBuilder(testPlatform), {
        'a|lib/a.module.library': ModuleLibrary.fromSource(assetA, "import 'b.dart';").serialize(),
